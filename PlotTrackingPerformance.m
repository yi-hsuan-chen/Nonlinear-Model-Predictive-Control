...
% getting desired trajectory
traj = ReferenceTrajectory(time,flightpath);
xd = traj(1,:)'; yd = traj(2,:)'; zd = traj(3,:)';
phid = traj(4,:)'; thetad = traj(5,:)'; psid = traj(6,:)';


fig(2) = figure('Name','p2p_x'); ax(5) = gca;
p(13:14) = plot(time,x,'b',time,xd,'r--');
xlabel('time (sec)'); ylabel('$x$ (m)');

fig(3) = figure('Name','p2p_y'); ax(6) = gca;
p(15:16) = plot(time,y,'b',time,yd,'r--');
xlabel('time (sec)'); ylabel('$y$ (m)');

fig(4) = figure('Name','p2p_z'); ax(7) = gca;
p(17:18) = plot(time,z,'b',time,zd,'r--');
xlabel('time (sec)'); ylabel('$z$ (m)');

fig(5) = figure('Name','p2p_phi'); ax(8) = gca;
p(19) = plot(time,phi,'b');
xlabel('time (sec)'); ylabel('$\phi$ (rad)');

fig(6) = figure('Name','p2p_theta'); ax(9) = gca;
p(20) = plot(time,theta,'b');
xlabel('time (sec)'); ylabel('$\theta$ (rad)');

fig(7) = figure('Name','p2p_psi'); ax(10) = gca;
p(21) = plot(time,psi,'b');
xlabel('time (sec)'); ylabel('$\psi$ (rad)');


% fig(2) = figure('Name','p2p_x');
